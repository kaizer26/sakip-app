                ...
                <form action="{{ route('indikator.update', $indikator->id) }}" method="POST">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-12 mb-3">
                            ...
                            <textarea name="tujuan" class="form-control" rows="2" required>{{ old('tujuan', $indikator->tujuan) }}</textarea>
                        </div>
                        <div class="col-md-12 mb-3">
                            <label class="form-label fw-bold">Sasaran</label>
                            <textarea name="sasaran" class="form-control" rows="2" required>{{ old('sasaran', $indikator->sasaran) }}</textarea>
                        </div>
                        <div class="col-md-12 mb-3">
                            <label class="form-label fw-bold">Indikator Kinerja</label>
                            <input type="text" name="indikator_kinerja" class="form-control" value="{{ old('indikator_kinerja', $indikator->indikator_kinerja) }}" required>
                        </div>
                        <div class="col-md-4 mb-3">
                            <label class="form-label fw-bold">Jenis Indikator</label>
                            <select name="jenis_indikator" class="form-select" required>
                                <option value="IKU" {{ old('jenis_indikator', $indikator->jenis_indikator) == 'IKU' ? 'selected' : '' }}>IKU</option>
                                <option value="Proksi" {{ old('jenis_indikator', $indikator->jenis_indikator) == 'Proksi' ? 'selected' : '' }}>Proksi</option>
                            </select>
                        </div>
                        <div class="col-md-4 mb-3">
                            <label class="form-label fw-bold">Periode</label>
                            <select name="periode" class="form-select" required>
                                <option value="Triwulanan" {{ old('periode', $indikator->periode) == 'Triwulanan' ? 'selected' : '' }}>Triwulanan</option>
                                <option value="Tahunan" {{ old('periode', $indikator->periode) == 'Tahunan' ? 'selected' : '' }}>Tahunan</option>
                            </select>
                        </div>
                        <div class="col-md-4 mb-3">
                            <label class="form-label fw-bold">Tipe Indikator</label>
                            <select name="tipe" class="form-select" required>
                                <option value="Persen" {{ old('tipe', $indikator->tipe) == 'Persen' ? 'selected' : '' }}>Persen</option>
                                <option value="Non Persen" {{ old('tipe', $indikator->tipe) == 'Non Persen' ? 'selected' : '' }}>Non Persen</option>
                            </select>
                        </div>
                        <div class="col-md-4 mb-3">
                            <label class="form-label fw-bold">Satuan</label>
                            <input type="text" name="satuan" class="form-control" value="{{ old('satuan', $indikator->satuan) }}" required>
                        </div>
                        <div class="col-md-4 mb-3">
                            <label class="form-label fw-bold">Target Tahunan</label>
                            <input type="number" step="0.01" name="target_tahunan" class="form-control" value="{{ old('target_tahunan', $indikator->target_tahunan) }}" required>
                        </div>
                        <div class="col-md-4 mb-3">
                            <label class="form-label fw-bold">Tahun</label>
                            <input type="number" name="tahun" class="form-control" value="{{ old('tahun', $indikator->tahun) }}" required>
                        </div>
                        <div class="col-md-12 mb-3">
                            ...
                            <select name="pic_id" class="form-select select2">
                                ...
                                @foreach($pegawais as $p)
                                    <option value="{{ $p->id }}" {{ old('pic_id', $indikator->pic_id) == $p->id ? 'selected' : '' }}>
                                        {{ $p->nama }} ({{ $p->nip ?? 'No NIP' }})
                                    </option>
                                @endforeach
                            </select>
