                        ...
                        @forelse($ros as $key => $ro)
                            <tr>
                                ...
                                <td>
                                    ...
                                    <small class="text-muted d-block text-truncate" style="max-width: 250px;" title="{{ $ro->indikator->indikator_kinerja }}">
                                ...
                                <td>
                                    <div>Awal: <span class="text-primary fw-bold">Rp {{ number_format($ro->pagu_awal, 0, ',', '.') }}</span></div>
                                    <div>Real: <span class="text-success fw-bold">Rp {{ number_format($ro->pagu_realisasi, 0, ',', '.') }}</span></div>
                                </td>
                                <td class="text-end">
                                    <div class="btn-group">
                                        <a href="{{ route('tabel-ro.edit', $ro) }}" class="btn btn-sm btn-outline-primary" title="Edit">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <form action="{{ route('tabel-ro.destroy', $ro) }}" method="POST" class="d-inline" onsubmit="return confirm('Yakin ingin menghapus RO ini?');">
                                            @csrf
                                            @method('DELETE')
